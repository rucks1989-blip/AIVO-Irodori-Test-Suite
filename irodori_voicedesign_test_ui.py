...
def build_request_payload(form_values: dict[str, Any]) -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "caption": form_values["caption"].strip() or None,
    ...
    if form_values["speaker_mode"] == "speaker_name":
        speaker_name = form_values["speaker_name"].strip()
        if speaker_name:
            payload["speaker_name"] = speaker_name
    elif form_values["speaker_mode"] == "ref_embed":
...
def call_health(api_url: str) -> tuple[int | None, Any, str | None]:
    try:
        response = requests.get(f"{api_url.rstrip('/')}/health", timeout=30)
        try:
...
def call_voice(api_url: str, payload: dict[str, Any]) -> tuple[int | None, Any, str | None]:
    try:
        response = requests.post(f"{api_url.rstrip('/')}/voice", json=payload, timeout=REQUEST_TIMEOUT_SECONDS)
        try:
            return response.status_code, response.json(), None
        except ValueError:
            return response.status_code, response.text, None
...
def main() -> None:
    st.set_page_config(page_title="Irodori VoiceDesign Test UI", layout="wide")
    init_state()
    ...
    st.title("Irodori VoiceDesign Thin API Client")
    st.caption("この UI は VoiceDesign API に JSON を POST するだけの別系統クライアントです。")
    ...
    with col_left:
        ...
        with st.form("voicedesign_form", clear_on_submit=False):
            ...
            text = st.text_area("Text", value="や、やっぱりまずかったよね……ごめんねヒッキー。", height=180)
            caption = st.text_area("caption", value="明るい女子高生", height=120)
            speaker_mode = st.radio(
                "Speaker Input Mode",
                options=["speaker_name", "ref_embed", "ref_wav", "caption_only"],
                format_func=lambda value: {
                    "speaker_name": "speaker_name を使う",
                    "ref_embed": "ref_embed のフルパスを使う",
                    "ref_wav": "ref_wav のフルパスを使う",
                    "caption_only": "caption only (no ref)",
                }[value],
                horizontal=True,
            )
            speaker_name = ""
            ...
            sanitize_symbols = st.checkbox("sanitize_symbols", value=False)
            bottom_submit_clicked = st.form_submit_button("Generate", key="vd_generate_bottom", type="primary", disabled=st.session_state.is_generating, use_container_width=True)
        ...
        payload = build_request_payload(form_values) if seed_error is None else build_request_payload({**form_values, "use_seed": False})
        current_hash = payload_hash(payload)
        same_as_previous = st.session_state.last_request_hash is not None and current_hash == st.session_state.last_request_hash

        if seed_error is not None:
            st.error(seed_error)
        if same_as_previous:
            st.warning("直前と同じリクエストです。")
        if submit_clicked:
            if st.session_state.is_generating:
                st.warning("生成中です。")
            elif not payload.get("text", "").strip():
                st.error("Text が空です。")
            elif seed_error is not None:
                st.error(seed_error)
            else:
                st.session_state.is_generating = True
                st.session_state.last_request_started_at = now_iso()
                ...
                st.session_state.last_request_finished_at = now_iso()
                ...
                st.session_state.last_wall_time = response_json.get("wall_time") if isinstance(response_json, dict) else None
    ...
    with col_right:
        ...
        if isinstance(response_json, dict):
            ...
            st.write(f"used_ref_embed: {response_json.get('used_ref_embed')}")
            st.write(f"caption: {response_json.get('caption')}")
